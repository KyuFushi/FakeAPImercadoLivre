...
function gerarHistorico30Dias() {
    ...
    for (let dia = 30; dia >= 0; dia--) {
        const qtdVendas = Math.floor(Math.random() * 40) + 20;
        for (let i = 0; i < qtdVendas; i++) {
            const produto = produtos[Math.floor(Math.random() *
                produtos.length)];
...
setInterval(gerarVendaTempoReal, 10000);
function dashboard() {
    const total = vendas.reduce((a, b) => a + b.valor, 0);
    return {
        metricas: {
            vendas: vendas.length,
            faturamento: Number(total.toFixed(2)),
            ticketMedio: Number((total /
                vendas.length).toFixed(2)),
            periodo: "30 dias"
        },
        produtos,
        ultimasVendas: vendas.slice(0, 30),
